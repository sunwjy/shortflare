...
import { createLinks, type LinkQueryResult, type LinkResult } from "@shortflare/links";
...
export const linkRoutes = new Hono<ManagementEnvironment>();
...
for (const command of ["activate", "disable", "archive", "restore"] as const) {
  linkRoutes.post(`/links/:linkId/${command}`, (context) => executeStateCommand(context, command));
}
